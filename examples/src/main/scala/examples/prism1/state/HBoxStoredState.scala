...
case class HBoxStoredState(store: LSMStore, override val version: VersionTag) extends
  ...
  override def semanticValidity(tx: SimpleBoxTransactionPrism): Try[Unit] = HBoxStoredState.semanticValidity(tx)
...
object HBoxStoredState {
  def semanticValidity(tx: SimpleBoxTransactionPrism): Try[Unit] = tx.semanticValidity
