...
import examples.commons.{PublicKey25519NoncedBox, PublicKey25519NoncedBoxSerializer, SimpleBoxTransactionPrism, SimpleBoxTransactionPrismCompanion}
...
case class PosBlock(override val parentId: BlockId, //PoW block
  ...
  with TransactionsCarryingPersistentNodeViewModifier[SimpleBoxTransactionPrism] with ScorexLogging {
...
object PosBlockCompanion extends Serializer[PosBlock] with ScorexEncoding {
  ...
  override def parseBytes(bytes: Array[Byte]): Try[PosBlock] = Try {
    ...
    position = position + 4
    val txs: Seq[SimpleBoxTransactionPrism] = (0 until txsLength) map { _ =>
      val l = Ints.fromByteArray(bytes.slice(position, position + 4))
      val tx = SimpleBoxTransactionPrismCompanion.parseBytes(bytes.slice(position + 4, position + 4 + l)).get
