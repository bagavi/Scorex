...
case class SimpleBoxTransactionPrism(from: IndexedSeq[(PublicKey25519Proposition, Nonce)],
  ...
  override type M = SimpleBoxTransactionPrism
  ...
  override lazy val newBoxes: Traversable[PublicKey25519NoncedBox] = to.zipWithIndex.map { case ((prop, value), idx) =>
    val nonce = SimpleBoxTransactionPrism.nonceFromDigest(Blake2b256(prop.pubKeyBytes ++ hashNoNonces ++ Ints.toByteArray(idx)))
    PublicKey25519NoncedBox(prop, nonce, value)
  }

  override lazy val serializer = SimpleBoxTransactionPrismCompanion
...
object SimpleBoxTransactionPrism extends ScorexEncoding {

  implicit val simpleBoxEncoder: Encoder[SimpleBoxTransactionPrism] = (sbe: SimpleBoxTransactionPrism) =>
    Map(
  ...
  def create(w: HBoxWallet,
    ...
    SimpleBoxTransactionPrism(from.map(t => t._1 -> t._2), outputs, fee, timestamp)
  }
}


object SimpleBoxTransactionPrismCompanion extends Serializer[SimpleBoxTransactionPrism] {

  override def toBytes(m: SimpleBoxTransactionPrism): Array[Byte] = {
  ...
  override def parseBytes(bytes: Array[Byte]): Try[SimpleBoxTransactionPrism] = Try {
    ...
    SimpleBoxTransactionPrism(from, to, signatures, fee, timestamp)
  }
}
