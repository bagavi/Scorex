...
class PowBlockHeader(
  ...
  def correctWork(difficulty: BigInt, s: BitcoinMiningSettings): Boolean = {
...
object PowBlockHeader extends ScorexLogging{
  ...
  def correctWorkDone(id: ModifierId, difficulty: BigInt, s: BitcoinMiningSettings): Boolean = {
...
case class PowBlock(override val parentId: BlockId,
                    ...
                    override val transactions: Seq[SimpleBoxTransactionBitcoin],
  ...
  extends PowBlockHeader(parentId, timestamp, nonce, generatorProposition, transactions.length, txsHash, minerId )
    with BitcoinBlock with ScorexLogging {
...
object PowBlockCompanion extends Serializer[PowBlock] with ScorexEncoding {
  ...
  def txBytes(transactions: Seq[SimpleBoxTransactionBitcoin]): Array[Byte] = {
  ...
  override def parseBytes(bytes: Array[Byte]): Try[PowBlock] = {
    ...
    PowBlockHeader.parse(headerBytes).flatMap { header =>
      Try {
        val txs: Seq[SimpleBoxTransactionBitcoin] = (0 until header.txsCount) map { _ =>
          val l = Ints.fromByteArray(bytes.slice(position, position + 4)) // Contains the length of the transaction
          position = position + 4
          val tx = SimpleBoxTransactionBitcoinCompanion.parseBytes(bytes.slice(position, position + l)).get
          position = position + l
          tx
        }
