...
trait ModifierGenerators {
  ...
  /**
    ...
    * - generate txCount transactions, each transaction has insPerTx inputs chosen from current state. the address of the inputs is generated by a function privKey in HybridGenerators, which is a random priv-pub key pair. The transactions are signed.
  ...
  def validPowBlocks(state: HBoxStoredState, parentIds: Seq[ModifierId]): Seq[PowBlock] = {
    ...
    val txs = stateBoxes.tail.grouped(insPerTx).map { inputs =>
      val fee = 0
      // G: don't understand the use of privKey
