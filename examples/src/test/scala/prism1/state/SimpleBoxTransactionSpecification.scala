...
import examples.commons.{Nonce, PublicKey25519NoncedBox, SimpleBoxTransactionPrism, Value}
...
class SimpleBoxTransactionSpecification extends PropSpec
  ...
  property("Transaction boxes are deterministic") {
    ...
    val tx = SimpleBoxTransactionPrism(IndexedSeq(genesisAccount -> Nonce @@ 0L), icoMembers.map(_ -> GenesisBalance), 0L, 0L)
  ...
  property("Generated transaction is valid") {
    forAll(simpleBoxTransactionPrismGen) { tx =>
